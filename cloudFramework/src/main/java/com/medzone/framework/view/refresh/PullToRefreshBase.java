...
import com.medzone.framework.view.refresh.library.LoadingLayout;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
	...
	protected int mState = PULL_TO_REFRESH;
	...
	protected void onScrollToBottomRefresh() {
		if (mMode.canPullUp()) {
			mCurrentMode = Mode.PULL_UP_TO_REFRESH;
			if (mState == PULL_TO_REFRESH) {
				if (mOnRefreshListener2 != null && isReadyForPullUp()) {
	...
	protected void updateUIForMode() {
		...
		// set it to pull down
		mCurrentMode = (mMode != Mode.BOTH) ? mMode : Mode.PULL_UP_TO_REFRESH;
	}
