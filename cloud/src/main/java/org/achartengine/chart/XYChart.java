...
/** The multiple series dataset. */
  ...
  /** The visible chart area, in screen coordinates.可见图区域，在 屏幕的坐标*/
   ...
   * @param width the width of the view to draw to View
   * @param height the height of the view to draw to View
  ...
  public void draw(Canvas canvas, int x, int y, int width, int height, Paint paint) {
    ...
    // use a linked list for these reasons: 使用链表的原因
    // 1) Avoid a large contiguous memory allocation 1)避免大量的内存分配
    // 2) We don't need random seeking, only sequential reading/writing, so  2)不需要随机读取，只需要按顺序来读写
    ...
    for (int i = 0; i < maxScaleNumber; i++) {
      ...
      for (int j = 0; j < length; j++) {
        ...
        if (or == Orientation.HORIZONTAL) {
          ...
          if (showGridX) {
            paint.setColor(mRenderer.getGridColor());
            PathEffect effect = new DashPathEffect(new float[]{2,4,8,16},20);
