...
import org.achartengine.tools.Gesture;
 ...
 */
public class TouchHandler extends SimpleOnGestureListener implements ITouchHandler {
  /** The chart renderer. 图表呈现器。(DefaultRenderer装的是图标的一些属性) */
  private DefaultRenderer mRenderer;
  /** The old x coordinate. X轴老坐标x */
  private float oldX;
  /** The old y coordinate. Y轴老坐标y */
  private float oldY;
  /** The old x2 coordinate. X轴老坐标x2 */
  private float oldX2;
  /** The old y2 coordinate. Y轴老坐标y2 */
  private float oldY2;
  /** The zoom buttons rectangle. 缩放按钮的矩形区域 */
  private RectF zoomR = new RectF();
  /** The pan tool. Pan工具类 */
  private Pan mPan;
  /** The zoom for the pinch gesture.捏的缩放手势。 */
  private Zoom mPinchZoom;
  /** The graphical view. 封装了图形化图表View 用于 显示在 GraphicalActivity */
  ...
  public TouchHandler(GraphicalView view, AbstractChart chart) {
    ...
    mGesture = new Gesture(chart, graphicalView.getContext(), this);
  }
  ...
  public boolean handleTouch(MotionEvent event) {
    ...
    mGesture.dispatchTouchEvent(event);
    ...
    if (mRenderer != null && action == MotionEvent.ACTION_MOVE) {
      if (oldX >= 0 || oldY >= 0) {
        ...
        // event.getPointerCount() 获取当前触控点的个数 也就是几个手指头在操作
        if (event.getPointerCount() > 1 && (oldX2 >= 0 || oldY2 >= 0) && mRenderer.isZoomEnabled()) {
          ...
          float zoomRate = 1;
          // 计算第一个手指滑动以后的缩放比例 (y轴偏移量/x轴偏移量)
          float tan1 = Math.abs(newY - oldY) / Math.abs(newX - oldX);
          // 计算第二个手指滑动以后的缩放比例 (y轴偏移量/x轴偏移量)
  ...
  /**
   * zoomRate： 缩放倍数 axis： 缩放的模式 ZOOM_AXIS_X ZOOM_AXIS_Y ZOOM_AXIS_XY (应该没错)
   */
  ...
  public void addSingleTapListener(SingleTapListener listener) {
    if (mGesture != null)
      mGesture.addSingleTapListener(listener);
  }

  @Override
  public void removeSingleTapListener(SingleTapListener listener) {
    if (mGesture != null)
      mGesture.removeSingleTapListenerr(listener);
  }
  ...
  public boolean onSingleTapConfirmed(MotionEvent e) {
    ...
    if (mGesture != null) {
      mGesture.notifySingleTapListener(e);
    }
    return true;
  }
